export async function extractDataFromSSEStream(stream: ReadableStream<Uint8Array>): Promise<string> {
	...
	try {
		while (true) {
			...
			while ((eolIndex = buffer.indexOf('\n')) >= 0) {
				...
				} else if (line === "" && currentData !== "") { // Empty line means the end of an event's data
					dataLines.push(currentData);
					currentData = "";
